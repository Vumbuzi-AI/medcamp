defmodule MedcampWeb.NursesPages.NurseNoteIndex do
  ...
  defp load_nurse_notes(socket) do
    ...
    total_count = NurseNotes.count_nurse_notes_by_nurse(nurse_id, search)
    ...
    nurse_notes =
      NurseNotes.list_nurse_notes_by_nurse_paginated(
        nurse_id,
        search,
        page,
  ...
  def render(assigns) do
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
      <.header class="text-[#373896] border-b border-gray-100 pb-4 mb-4">
        <div class="flex items-center">
          ...
          Listing My Nurse Notes
        </div>
      </.header>

      <form phx-change="search" class="mb-4">
        <.search_input name="search" value={@search} placeholder="Search by patient name or content" />
      </form>
      ...
      <.blank_state
        ...
        title="No nurse notes found"
        description={
          if @search != "",
            do: "No nurse notes match \"#{@search}\".",
            else: "You haven't created any nurse notes yet."
        }
      >
        <:actions :if={@search != ""}>
        ...
        >
          ...
          <:action :let={nurse_note}>
            <div class="flex items-center justify-center">
              <.link
                patch={~p"/nurse/nurse_notes/#{nurse_note}/edit"}
    ...
    <.modal
      ...
      id="nurse_note-modal"
      show
      on_cancel={JS.patch(~p"/nurse/nurse_notes")}
    >
      <.live_component
