defmodule MedcampWeb.AdminGeneralInventoryLive.Index do
  ...
  @impl true
  def handle_info(
        {MedcampWeb.AdminGeneralInventoryLive.FormComponent, {:saved, _general_inventory_item}},
        socket
      ) do
    {:noreply, load_general_inventory_items(socket)}
  end
  ...
  def handle_event("paginate", %{"page" => page}, socket) do
    {:noreply,
     socket |> assign(:page, max(1, String.to_integer(page))) |> load_general_inventory_items()}
  end
  ...
  defp load_general_inventory_items(socket) do
    items =
  ...
  def render(assigns) do
    all_items =
      if assigns.search_query != "" do
        Inventories.search_general_inventory_items(assigns.search_query)
      else
        Inventories.list_general_inventory_items()
      end

    assigns = assign(assigns, :stock_counts, get_stock_counts(all_items))
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
      <.header class="text-[#373896] border-b border-gray-100 pb-4 mb-4">
        ...
        <:actions>
          <.link patch={~p"/admin/general_inventory/new"}>
      ...
      <% else %>
        <.table
          ...
          row_click={fn item -> JS.navigate(~p"/admin/general_inventory/#{item}") end}
          row_id={&"general_inventory_items-#{&1.id}"}
        >
          <:col :let={item} label="Item Details">
            <div class="flex flex-col items-start py-3">
              <span class="font-medium text-gray-900 text-base">{item.name}</span>
              <span class="text-sm text-gray-600">{item.category}</span>
              <span class="text-sm text-gray-600">{item.gtin || "N/A"}</span>
            </div>
          </:col>
          ...
          <:action :let={item}>
            <.link
              navigate={~p"/admin/general_inventory/#{item}"}
          ...
          <:action :let={item}>
            <.link
              patch={~p"/admin/general_inventory/#{item}/edit"}
              class="flex items-center text-[#6667ab] hover:text-[#373896]"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
      ...
      <.modal
        ...
        show
        on_cancel={JS.patch(~p"/admin/general_inventory")}
      >
        <.live_component
          ...
          patch={~p"/admin/general_inventory"}
        />
      </.modal>
    </div>
    """
