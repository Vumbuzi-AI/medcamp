defmodule MedcampWeb.AdminNurseAllocationsLive.Index do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     socket
     |> assign(:active_tab, :admin_nurse_allocations)
     |> assign(:users, users)
     |> assign(:nurse_filters, default_nurse_filters())
     |> assign(:nurse_query_filters, %{})
     |> assign(:search, "")
     |> assign(:page, 1)
     |> assign(:per_page, @per_page)
     |> load_nurse_allocations()
     |> assign(:quote_request_modal, nil)}
  end

  defp default_nurse_filters do
    %{
      "date_from" => "",
      "date_to" => "",
      "allocated_by_id" => "",
  ...
  # wiping it out.
  @impl true
  def handle_event("filter_nurse", %{"filters" => filters}, socket) do
    ...
    {:noreply,
     socket
     |> assign(:nurse_filters, filters)
     |> assign(:nurse_query_filters, params)
     |> assign(:page, 1)
     |> load_nurse_allocations()}
  end
  ...
  def handle_event("search", %{"search" => term}, socket) do
    query_filters = Map.put(socket.assigns.nurse_query_filters, :search, term)

    {:noreply,
     socket
     |> assign(:search, term)
     |> assign(:nurse_query_filters, query_filters)
     |> assign(:page, 1)
     |> load_nurse_allocations()}
  end

  def handle_event("clear_filters_nurse", _, socket) do
    {:noreply,
     socket
     |> assign(:nurse_filters, default_nurse_filters())
     |> assign(:nurse_query_filters, %{})
  ...
  def handle_event("clear_chip", %{"field" => field}, socket) do
    filters = Map.put(socket.assigns.nurse_filters, field, "")
    handle_event("filter_nurse", %{"filters" => filters}, socket)
  end

  def handle_event("open_quote_modal_nurse", %{"id" => id}, socket) do
    na = Nursing.get_nursing_allocation!(id)
    {item_name, supplier} = item_and_supplier_for_nurse_allocation(na)
  ...
  def handle_event("paginate", %{"page" => page}, socket) do
    {:noreply,
     socket |> assign(:page, max(1, String.to_integer(page))) |> load_nurse_allocations()}
  end
  ...
  defp item_and_supplier_for_nurse_allocation(na) do
    item =
      if na.inventory_issued && na.inventory_issued.inventory_received do
        na.inventory_issued.inventory_received.brand_name ||
          na.inventory_issued.inventory_received.generic_name || "Item"
      else
        "Nursing allocation ##{na.id}"
      end

    supplier =
      if na.inventory_issued && na.inventory_issued.batch && na.inventory_issued.batch.supplier do
        s = na.inventory_issued.batch.supplier
  ...
  defp load_nurse_allocations(socket) do
    filters = socket.assigns.nurse_query_filters
    per_page = socket.assigns.per_page
    page = socket.assigns.page

    total_count = Nursing.count_nursing_allocations(filters)
    total_pages = Medcamp.Pagination.total_pages(total_count, per_page)
    page = min(max(1, page), total_pages)
    nursing_allocations = Nursing.filter_nursing_allocations_paginated(filters, page, per_page)

    socket
    |> assign(:page, page)
    |> assign(:total_count, total_count)
    |> assign(:total_pages, total_pages)
    |> assign(:nursing_allocations, nursing_allocations)
  end
      ...
      <div class="space-y-4">
        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
          <.page_header
            ...
            title="Nurse Allocations"
            subtitle="Filter and manage nurse allocations."
          />
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <.filter_drawer
              id="nurse-allocations-filters"
              title="Filter nurse allocations"
              apply_event="filter_nurse"
              clear_event="clear_filters_nurse"
              active_count={count_active_filters(@nurse_filters)}
            >
              <:group label="Date Range">
                <.date_range_fields
                  from_name="filters[date_from]"
                  to_name="filters[date_to]"
                  from_value={@nurse_filters["date_from"]}
                  to_value={@nurse_filters["date_to"]}
                />
              </:group>
              ...
              <:group label="Allocated By / To">
                <div>
                  ...
                  >
                    ...
                    <%= for {email, id} <- @users do %>
                      <option value={id} selected={@nurse_filters["allocated_by_id"] == to_string(id)}>
                        {email}
                      </option>
                    <% end %>
                  </select>
                ...
                <div>
                  ...
                  >
                    <option value="">All</option>
                    <%= for {email, id} <- @users do %>
                      <option value={id} selected={@nurse_filters["allocated_to_id"] == to_string(id)}>
                        {email}
                      </option>
                    <% end %>
                  </select>
              ...
              <:group label="Expiry">
                <.expiry_filter_fields
                  status_value={@nurse_filters["expiry_status"]}
                  from_value={@nurse_filters["expiry_from"]}
                  to_value={@nurse_filters["expiry_to"]}
                />
              </:group>

              <:chip
                :for={chip <- filter_chips(@nurse_filters, @users)}
                label={chip.label}
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-x-auto">
          <.table id="nursing_allocations" rows={@nursing_allocations}
            row_id={&"nursing_allocations-#{&1.id}"}
          >
            <:empty_state>
              <tr>
                <td class="px-6 py-4 text-sm">
                  <p class="font-semibold text-gray-900">
                    {if @search != "" or count_active_filters(@nurse_filters) > 0,
                      do: "No nurse allocations match the current filters",
                      else: "No nurse allocations available"}
                  </p>
            ...
            <:col :let={na} label="Item">
              <%= if na.inventory_issued && na.inventory_issued.inventory_received do %>
                {na.inventory_issued.inventory_received.brand_name ||
                  na.inventory_issued.inventory_received.generic_name || "—"}
              <% else %>
                —
              <% end %>
            </:col>
            <:col :let={na} label="Allocated qty">{na.allocated_quantity}</:col>
            <:col :let={na} label="Remaining">
              <span class={
                if (na.remaining_quantity || 0) <= 20, do: "text-red-600 font-semibold", else: ""
              }>
                {na.remaining_quantity}
              </span>
              <%= if (na.remaining_quantity || 0) == 0 do %>
                <span class="text-red-600 text-xs ml-1">(out)</span>
              <% else %>
                <%= if (na.remaining_quantity || 0) <= 20 do %>
                  <span class="text-red-600 text-xs ml-1">(low)</span>
                <% end %>
              <% end %>
            </:col>
            <:col :let={na} label="UOM">{na.uom || "—"}</:col>
            <:col :let={na} label="Expiry">
              <%= if na.expiry_date && StockAlerts.near_expiry?(na.expiry_date) do %>
                <span class="text-red-600 font-semibold" title="Expiring within 90 days">
                  {format_expiry(na.expiry_date)}
                </span>
            ...
            <:col :let={na} label="Allocated by">
              {(na.allocated_by_user && na.allocated_by_user.name) || "—"}
            </:col>
            <:col :let={na} label="Allocated to">
              {(na.allocated_to_user && na.allocated_to_user.name) || "—"}
            </:col>
            <:col :let={na} label="Date">{format_date(na.inserted_at)}</:col>
            <:action :let={na}>
              <button
                type="button"
                phx-click="open_quote_modal_nurse"
                phx-value-id={na.id}
