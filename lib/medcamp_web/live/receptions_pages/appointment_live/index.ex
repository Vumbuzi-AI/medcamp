defmodule MedcampWeb.ReceptionsPagePatientLive.AppointmentIndex do
  ...
  defp load_appointments(socket) do
    filters = %{patient_search: socket.assigns.search}
    ...
    appointments =
      Appointments.list_appointments_paginated(filters, page, socket.assigns.per_page)
  ...
  def handle_event("search", %{"search" => term}, socket) do
    {:noreply, socket |> assign(:search, term) |> assign(:page, 1) |> load_appointments()}
  end

  @impl true
  def handle_event("clear_search", _params, socket) do
    {:noreply, socket |> assign(:search, "") |> assign(:page, 1) |> load_appointments()}
  end
  ...
  @impl true
  def handle_info({:appointment_updated, appointment}, socket) do
    appointments =
      Enum.map(socket.assigns.appointments, &if(&1.id == appointment.id, do: appointment, else: &1))

    {:noreply, assign(socket, :appointments, appointments)}
  end

  @impl true
  def handle_info({:appointment_deleted, _appointment}, socket) do
    {:noreply, load_appointments(socket)}
  end
  ...
  def render(assigns) do
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
      <.header class="text-[#373896] border-b border-gray-100 pb-4 mb-4">
        ...
        <:actions>
          <.link patch={~p"/reception/appointments/new"}>
