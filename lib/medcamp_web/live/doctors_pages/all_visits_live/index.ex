defmodule MedcampWeb.DoctorsPagePatientLive.VisitsIndex do
  ...
  def handle_event("search", %{"search" => term}, socket) do
    {:noreply, socket |> assign(:search, term) |> assign_visits(1)}
  end

  @impl true
  def handle_event("clear_search", _params, socket) do
    {:noreply, socket |> assign(:search, "") |> assign_visits(1)}
  end
  ...
  defp assign_visits(socket, page) do
    page = normalize_page(page)
    filters = %{search: socket.assigns.search}

    total_count =
      PatientVisits.count_patient_visits_by_doctor_id(socket.assigns.current_user.id, filters)

    total_pages = max(1, div(total_count + @per_page - 1, @per_page))
    page = min(page, total_pages)

    patient_visits =
      PatientVisits.list_patient_visits_by_doctor_id_paginated(
  ...
  def render(assigns) do
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
      <.page_header
        ...
        title="Your Visits"
        subtitle="Search your patient visits."
      />
      ...
      <.blank_state
        ...
        description={
          if @search != "",
            do: "No visits match the current search.",
            else: "You have no patient visits recorded yet."
        }
      >
        <:actions :if={@search != ""}>
